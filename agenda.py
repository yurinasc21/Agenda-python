    ...
    *********** SEJA BEM VINDO !! ************ 
    ...
    ''')
        if opcao =="1":
            ...
            sair()
        voltarMenuPrincipal=input("Deseja voltar ao menu principal ? (digite 's' para sim ou 'n' para não)"'\n').lower()
...
def consultarCompromisso():
    data=input(f'Digite a data do compromisso a ser consultada (xx/xx/xxxx): ')
    agenda = open("agenda.txt","r")
    for compromisso in agenda:
